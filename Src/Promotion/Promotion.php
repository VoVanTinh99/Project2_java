...
$list_Promotion= mysqli_query($conn,$sqlSelect);

?>
<h3 class="w3_inner_tittle two text-center">Manage Promotion</h3>
<a class="btn btn-primary" href="?page=AddPromotion">Add <i class="fa fa-plus"></i></a>
<br/>
<br/>
<table id="myTable" class="table-striped table-hover">
	...
	<tbody>
		<?php
		$num = 1;
		while(list($PromotionId, $name, $discount, $content, $active, $close, $open) = mysqli_fetch_array($list_Promotion))
		{
			?>
			<tr>
				...
				<td class="col-md-1" style="text-align:center;">
					<?= ($open == 0) ? '<a class="btn btn-default" title="Đang ngừng. Click to start promotions!" href="?page=ChangeActive&PromotionId='.$PromotionId.'&Do=OK"><span class="glyphicon glyphicon-ok" style="color:blue;"></span></a>' : '<a class="btn btn-default" title="Đang diễn ra. Click to stop promotion!" href="?page=ChangeActive&PromotionId='.$PromotionId.'&Do=Remove"><span class="glyphicon glyphicon-remove" style="color:red;"></span></a>';?>
				</td>
