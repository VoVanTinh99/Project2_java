...
$list_order =  mysqli_query($conn,$cmd);
...
if(isset($_GET['order_id']))
{
    ...
    {   
        mysqli_query($conn,"UPDATE `order` SET OrderStatus = 1 WHERE OrderId = ".$_GET['order_id']);
    ...
    <table id="tableluna" class="table table-striped table-responsive" cellpadding="0" width="" >          
      ...
      <tbody>
          ...
          while(list($id, $username, $create, 
                     $deliver, $status, $payment) = mysqli_fetch_array($list_order))
          {
          ?>
            <tr>
              <td><?= $id;?></td>
